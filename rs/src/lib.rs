...
If the substrait submodule directory exists, the
...
in sync. The substrait submodule is not included in the crate manifest, so this
...
The process for Python is much the same, but handled directly in the
[build.rs file for the Python module](https://github.com/substrait-io/\
substrait-validator/blob/main/py/build.rs). The substrait submodule check is
used to determine whether we're building from the git repository or from a
source distribution.
