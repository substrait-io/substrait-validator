...
//!
//! Protobuf types are generated by [`prost`] via `build.rs`. The build script
//! enables `prost::Name` (for message type names) and embeds a
//! `file_descriptor_set.bin` for runtime reflection via `prost-reflect`.
//!
//! [`DESCRIPTOR_POOL`] provides access to the embedded descriptor at runtime.
//! It is used by the derive-generated [`prost_reflect::ReflectMessage`] impls
//! for descriptor lookup, and by the [`ProtoEnum`] impls for enum type and
//! variant name resolution.
//!
//! [`field_descriptor_to_node`] maps a [`prost_reflect::FieldDescriptor`] to a
//! [`tree::Node`], used by [`parse_proto_message_unknown`] to represent
//! unrecognized fields in the output tree.
//!
