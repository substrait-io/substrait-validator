...
re-parses each test every time it's run. At present, this still happens in the
blink of an eye, but as this slows down, it should be made smarter.
...
To run the tests, you will need:
 ...
 - Python 3.x, with `substrait-protobuf`, `pyyaml`, and `click` installed.
   Installing the validator's Python bindings (`pip install ../py`) pulls all
   three in.
