...
def run(filter, release, html):
    ...
    # Earlier versions of this script generated the Substrait protobuf
    # bindings here with protoc; remove them if they're still around, as they
    # would shadow the substrait-protobuf package below.
    script_path = os.path.dirname(os.path.realpath(__file__))
    stale_proto_path = os.path.join(script_path, "substrait")
    if os.path.isdir(stale_proto_path):
        click.echo("Removing previously generated protobuf bindings...")
        shutil.rmtree(stale_proto_path)

    # Import the Substrait protobuf bindings. These are provided by the
    # substrait-protobuf package, which is published from the specification
    # alongside the crates that the validator itself is built against, so
    # there is nothing to generate here.
    try:
        from substrait import plan_pb2
    except ImportError:
        click.echo(
            "Could not import the Substrait protobuf bindings. Install the "
            "substrait-protobuf package (see README.md)."
        )
        raise
...
def clean():
    ...
    # Remove protobuf bindings generated by earlier versions of this script.
